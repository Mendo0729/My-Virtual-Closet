...
IMAGE="${1:-}"
RUNS="${2:-5}"
URL="${GEMINI_TEST_URL:-http://100.74.236.102:5173/ai/segment-garment}"
...
for i in $(seq 1 "$RUNS"); do
  ...
  read -r API_MS LABEL MASK_POINTS <<< "$(python3 - "$TMP_RESPONSE" <<'PY'
import json, sys
try:
    with open(sys.argv[1], encoding='utf-8') as fh:
        data = json.load(fh)
    segmentation = data.get('segmentation') or {}
    label = str(segmentation.get('label', '-')).replace(' ', '_')
    mask = segmentation.get('mask') or []
    print(data.get('latencyMs', '-'), label, len(mask))
except Exception:
    print('-', '-', '-')
PY
)"

  printf 'Prueba %d | HTTP %s | TTFB %.2fs | Total %.2fs | Gemini %sms | %s | mask=%s pts\n' \
    "$i" "$HTTP" "$TTFB" "$TOTAL" "$API_MS" "$LABEL" "$MASK_POINTS"
done
